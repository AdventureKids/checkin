...
function generatePinFromBirthday(birthday) {
  if (!birthday) return Math.floor(100000 + Math.random() * 900000).toString();
  const date = new Date(birthday);
  if (isNaN(date.getTime())) return Math.floor(100000 + Math.random() * 900000).toString();
...
const rawData = [
  ...
  // More families from later pages
...
console.log(`Found ${guardians.length} guardians and ${children.length} children`);
...
for (const guardian of guardians) {
  ...
  if (!phone) {
    console.log(`Skipping guardian ${guardian.firstName} ${guardian.lastName} - no valid phone`);
    continue;
  }
  
  const key = `${guardian.firstName.toLowerCase()}_${guardian.lastName.toLowerCase()}`;
  const familyName = `The ${guardian.lastName.charAt(0).toUpperCase() + guardian.lastName.slice(1).toLowerCase()} Family`;
...
for (const child of children) {
  ...
  if (family) {
    ...
    family.children.push({
      firstName: child.firstName.charAt(0).toUpperCase() + child.firstName.slice(1).toLowerCase(),
      lastName: child.lastName.charAt(0).toUpperCase() + child.lastName.slice(1).toLowerCase(),
      birthday: birthday,
      age: age,
    ...
    console.log(`No family found for child ${child.firstName} ${child.lastName} (guardian: ${child.guardianFirst} ${child.guardianLast})`);
  }
}
...
for (const family of familiesWithChildren) {
  ...
  if (existing) {
    console.log(`Skipping ${family.name} - phone ${family.phone} already exists`);
    skipped++;
    continue;
  }
...
console.log(`  Imported: ${imported} families`);
